...
// falhava, e nenhuma leitura RS-485 era jamais decodificada.
describe("Rs485Connection — montagem de linhas a partir de chunks parciais", () => {
    ...
    it("linha que não é JSON válido chega como { raw, port, timestamp } — diferente do RS-232, inclui a porta", () => {
        const connection = new Rs485Connection({
            meterId: "meter-rs485-test-3",
            address: "/dev/ttyUSB0",
        }) as unknown as SerialDataHarness
        const dataHandler = vi.fn()
        connection.onData(dataHandler)
