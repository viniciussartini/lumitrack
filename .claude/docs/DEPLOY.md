...
- **Correção:** já resolvido no repositório (ADR-0010) — `VITE_SSE_URL` no `render.yaml` aponta o frontend direto para a origem da API só para essa chamada, cross-origin. Isso exigiu `sameSite: "none"` no cookie de sessão em produção (`shared/security/csrf.ts`) **e** liberar essa origem no CSP do próprio `index.html` (`VITE_CSP_CONNECT_EXTRA`, também no `render.yaml`) — sem essa segunda variável, o navegador bloqueia a conexão sozinho, via CSP, antes mesmo de CORS entrar em jogo (erro no console: "Refused to connect because it violates the document's Content Security Policy"). Se você mudar o nome do serviço da API no Render, atualize as **três** referências junto: `VITE_SSE_URL`, `VITE_CSP_CONNECT_EXTRA` e o destino do rewrite (item 4 do passo a passo).
