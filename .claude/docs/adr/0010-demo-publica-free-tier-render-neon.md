...
- **Limite de 0,5 GB no Neon.** O seed atual gera 1 ano de leituras por minuto (~2,1 milhões de linhas, ~650 MB) e **não caberia**; a demo exige um seed reduzido. O `RetentionService` ainda não cobre `MeterReading` (item da Fase 14), então o crescimento das leituras ao vivo precisa de acompanhamento manual até lá.
