...
2. **Rewrite `/api/*` no site estático apontando para o web service.** O frontend chama a API por caminho relativo (`services/api.ts`). O rewrite mantém tudo na mesma origem do ponto de vista do browser, preservando **sem uma linha de mudança** o cookie de sessão `HttpOnly` e a diretiva `connect-src 'self'` da CSP, para toda chamada REST comum.
   ...
   **Exceção descoberta em produção, não antecipada no desenho original: o stream SSE (`GET /api/iot/stream`, `lib/sse/appStream.ts`) trava sem entregar nenhum dado através desse rewrite** — confirmado isolando a causa com `curl -N` direto no serviço da API (funciona) vs. através do rewrite do site estático (trava, sem sequer devolver headers de resposta ao browser). Rewrite de site estático no Render não sustenta conexão de longa duração; foi desenhado para redirecionar página, não para proxiar streaming.
   ...
   A correção: só essa chamada passa a apontar direto para a origem real da API (`VITE_SSE_URL` em `render.yaml`, cross-origin de verdade), com `credentials: "include"`. Isso exige que o cookie de sessão trafegue cross-site, o que forçou `sameSite: "none"` em produção (`shared/security/csrf.ts`) — a consequência que este item originalmente listava como motivo para **não** servir a API em outro domínio. Julgada segura porque: (a) CSRF só é avaliado para métodos que mudam estado (`authenticate.ts`, `SAFE_METHODS`) — o stream é `GET`; (b) toda requisição que muda estado continua no mesmo-origin via rewrite, com o double-submit CSRF intacto e sem nenhuma mudança. `sameSite:"none"` exige `secure:true`, que só é `true` em produção — fora dela (`development`/`test`) o cookie continua `lax`, sem nenhum cross-origin a sustentar.

   Segunda camada que também precisou ceder, descoberta só depois de corrigir a primeira: o CSP do próprio `frontend/index.html` (`connect-src 'self'`) bloqueia a conexão no navegador **antes** de CORS entrar em jogo — SameSite/CORS corretos não bastam se o CSP não abrir a origem. `VITE_CSP_CONNECT_EXTRA` (`render.yaml`, injetado via `%VITE_CSP_CONNECT_EXTRA%` no `<meta>` do CSP — recurso nativo de `index.html` do Vite) libera especificamente a origem da API em `connect-src`, só na demo do Render; vazio por padrão em dev/self-hosted (default definido em `vite.config.ts`, já que Vite deixa o placeholder literal se a variável não existir).
...
- **Cookie de sessão com `sameSite:"none"` em produção** (2026-08-10), só para sustentar o stream SSE cross-origin (ver item 2 acima) — perde a camada de proteção que `Lax` dava contra cross-site de graça; o double-submit CSRF continua cobrindo toda requisição que muda estado, então o risco residual é aceito, não ignorado. Não se aplica fora do Caminho A (self-hosted continua `lax`, sem cross-origin nenhum).
