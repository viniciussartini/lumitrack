...
2. **Rewrite `/api/*` no site estático apontando para o web service.** O frontend chama a API por caminho relativo (`services/api.ts`). O rewrite mantém tudo na mesma origem do ponto de vista do browser, preservando **sem uma linha de mudança** o cookie de sessão `HttpOnly` e a diretiva `connect-src 'self'` da CSP, para toda chamada REST comum.

   **Exceção descoberta em produção, não antecipada no desenho original: o stream SSE (`GET /api/iot/stream`, `lib/sse/appStream.ts`) trava sem entregar nenhum dado através desse rewrite** — confirmado isolando a causa com `curl -N` direto no serviço da API (funciona) vs. através do rewrite do site estático (trava, sem sequer devolver headers de resposta ao browser). Rewrite de site estático no Render não sustenta conexão de longa duração; foi desenhado para redirecionar página, não para proxiar streaming.

   Primeira tentativa de correção — descartada — foi apontar essa chamada direto pra origem real da API (`VITE_SSE_URL`), cross-origin, com `credentials: "include"` e `sameSite: "none"` no cookie de sessão. Não funcionou: o cookie foi definido pelo **navegador** para o domínio do site estático (`lumitrack.onrender.com`), não para o da API — `SameSite` controla política cross-*site*, não decide isso, é o `Domain` do cookie que nunca inclui o outro serviço. Setar `Domain=.onrender.com` também não é opção: `onrender.com` está na lista de sufixos públicos (igual `vercel.app`), o navegador rejeita esse cookie de propósito, pra impedir vazamento entre inquilinos diferentes da mesma plataforma. Resultado observado: a conexão chegava na API (depois de resolver o bloqueio de CSP abaixo) mas voltava 401 — sem cookie nenhum chegando.

   Segunda camada que também precisou ceder, descoberta entre as duas tentativas: o CSP do próprio `frontend/index.html` (`connect-src 'self'`) bloqueia a conexão no navegador **antes** mesmo de CORS ou cookie entrarem em jogo. `VITE_CSP_CONNECT_EXTRA` (`render.yaml`, injetado via `%VITE_CSP_CONNECT_EXTRA%` no `<meta>` do CSP — recurso nativo de `index.html` do Vite) libera especificamente a origem da API em `connect-src`, só na demo do Render; vazio por padrão em dev/self-hosted (default definido em `vite.config.ts`, já que Vite deixa o placeholder literal se a variável não existir). Essa parte continua necessária na correção final.

   **Correção final: ticket de uso único.** `POST /api/iot/stream-ticket` (`backend/src/modules/iot/iot-stream.routes.ts`) autentica normalmente — cookie, mesma origem, via rewrite — e emite um token opaco de 30s, uso único (`sse-ticket.service.ts`, em memória). O frontend troca esse ticket pela conexão SSE via query string (`GET /api/iot/stream?ticket=...`), sem depender de cookie nenhum nessa segunda chamada. `sameSite: "lax"` volta a ser suficiente em todo cookie da aplicação — o cookie nunca precisa atravessar domínio, porque a única chamada que atravessa não usa cookie. Reconexão automática do `fetchEventSource` é desligada no caminho cross-origin (um `onerror` que sempre relança) e substituída por um loop próprio em `lib/sse/appStream.ts` que busca um ticket **novo** a cada tentativa — reaproveitar a mesma URL como a lib faria sozinha falharia sempre, o ticket já teria sido consumido.
...
- **Ticket de uso único para o SSE** (2026-08-10, ver item 2 acima) — um componente novo de estado em memória (`SseTicketService`), pequeno mas real: se o processo reiniciar entre a emissão e o consumo do ticket (janela de 30s), a conexão falha e o cliente tenta de novo automaticamente. Trade-off aceito em troca de manter `sameSite: "lax"` em todo cookie da aplicação, sem exceção — nenhuma camada de proteção cedeu de verdade.
