...
| **23** | **Shell v2 — navegação redesenhada (Painel · Análise · Histórico · Relatórios · Configurações)** | Planejada — fase atual, objetivo abaixo |
...
Reestrutura a navegação do app logado para **Painel · Análise · Histórico · Relatórios · Alertas · Distribuidoras · Sobre**, mais **Configurações** (sub-nav Cadastro/Relatórios/Metas/Conta) — mesma classe de mudança estrutural da Fase 6 ("o que todo o resto herda vem antes"). Cria o componente de hierarquia navegável (Propriedade→Área→Dispositivo, com busca) que Análise (Fase 24) e Configurações→Cadastro compartilham; move Perfil/Segurança para Configurações→Conta; cria rotas-esqueleto para Histórico, Relatórios e Metas (populadas nas fases seguintes). Cobre RF46 (estrutura) e a parte estrutural de FNC008. **Depende de:** —. **Risco:** estrutural, mas contido — é rota e menu, não lógica de negócio; nenhuma tela de Grupo B ou Grupo A sofre regressão funcional.

### Fase 24 — Análise: consumo e custos

Reestrutura `/propriedades` na aba **"Consumo e Custos"** do handoff: dados do item + medidor, gráfico de consumo em tempo real, comparação lateral (Áreas↔Propriedade, Dispositivos↔Área, restrita às que têm medidor) e a área de análise configurável (janela, dia/hora, agregação, grandeza, botão "Gerar análise"). Cobre RF38 (parcial — grandeza limitada ao que já é medido hoje; THD/reativa/aparente ficam para a Fase 25) e FNC005 itens 1–3 e 5–6. **Não inclui** a aba "Grandezas Elétricas" — ver Fase 25. **Depende de:** Fase 23.
...
Lista os dispositivos/origens onde a conta está autenticada e permite encerrar qualquer sessão (ou todas as outras de uma vez). Cobre RF44 e FNC012. **Achado:** `AuthToken`/`RefreshToken` hoje guardam só canal e datas — nenhum device/IP/user-agent —, então a listagem de "dispositivo, origem" do RF44 fica limitada sem captar esse dado no login; decidir na chegada se channel+data basta ou se abre uma migração pra capturar essa informação. **Depende de:** Fase 23. **Risco:** baixo — é a fase de menor valor de produto do bloco (conveniência/segurança, não monitoramento de energia), por isso fica por último.
