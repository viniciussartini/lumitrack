...
| **24** | **Análise — consumo e custos (reestruturação de `/propriedades`)** | Planejada — fase atual, ainda em nível de objetivo |
...
Reestrutura `/propriedades` na aba **"Consumo e Custos"** do handoff: dados do item + medidor, gráfico de consumo em tempo real, comparação lateral (Áreas↔Propriedade, Dispositivos↔Área, restrita às que têm medidor) e a área de análise configurável (janela, dia/hora, agregação, grandeza, botão "Gerar análise"). Cobre RF38 (parcial — grandeza limitada ao que já é medido hoje; THD/reativa/aparente ficam para a Fase 25) e FNC005 itens 1–3 e 5–6. **Não inclui** a aba "Grandezas Elétricas" — ver Fase 25. **Depende de:** Fase 23. **Herda da Fase 23 (acordado em 2026-09-19):** (a) a árvore de seleção de Análise (busca "Buscar na hierarquia", linha selecionável) é UI própria — não reaproveita a árvore do Cadastro, que tem outro visual —, mas consome o mesmo endpoint/hook de árvore, estendido aqui com a flag "tem medidor" para restringir as comparações; (b) **critério de aceite:** ao reconstruir as páginas de detalhe e a lista, remover os pontos de entrada de criar/editar/excluir Propriedade/Área/Dispositivo (`PropertyMenu`, `AreaMenu`, `DeviceMenu`, botões "Nova …") — a Fase 23 deixa o Cadastro com duas entradas de propósito, e é aqui que o "concentrar" do RF46 fecha; o cadastro de **Medidor** permanece nas páginas de detalhe (é assim no design); (c) decidir se `/propriedades` é renomeada (ex.: `/analise`), cuidando dos deep-links e dos e2e.

### Fase 25 — Telemetria ampliada: grandezas elétricas por fase

Estende a ingestão (`IoTDataProcessor` + 8 adaptadores de protocolo) e o `MeterReading` para tensão por fase e fase-neutro média, desequilíbrio, corrente por fase e de neutro, potência reativa/aparente/por fase, frequência, fator de potência por fase e THD de tensão/corrente por fase — alimenta a aba "Grandezas Elétricas" de Análise. Cobre RF37 e RN34. **Abre com o spike que resolve a decisão aberta do `07`** ("grandezas medidas × calculadas pelo medidor") — sem essa resposta não dá pra fechar o schema: persistir um campo derivado como se fosse medido, ou recalcular o que o próprio medidor já entrega pronto, são erros de direção opostos e igualmente caros de reverter depois que houver histórico gravado. **Depende de:** Fase 24 (a aba vive dentro de Análise) e da decisão do `07`. **Risco:** alto — mesma classe da Fase 19 (mexe em pipeline usado por todo medidor já em produção). As fases seguintes deste bloco **não** dependem desta — se a decisão demorar, o roadmap segue.
