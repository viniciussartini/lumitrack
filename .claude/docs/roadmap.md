...
Estende a ingestão (`IoTDataProcessor` + 8 adaptadores de protocolo) e o `MeterReading` para tensão por fase e fase-neutro média, desequilíbrio, corrente por fase e de neutro, potência reativa/aparente/por fase, frequência, fator de potência por fase e THD de tensão/corrente por fase — alimenta a aba "Grandezas Elétricas" de Análise. **Recebe da Fase 24 (2026-09-20)** o seletor de abas de Análise e a **área de análise configurável** (janela Hora/Dia, dia, hora, agregação, grandeza, "Gerar análise", gráfico e tabela mín/média/máx — no design ela vive só nessa aba): o mínimo e o máximo do bucket vêm dos minutos dentro dele (o `MeterReading` guarda só médias por minuto; o protótipo simula ±1,2% da média), e a lista de grandezas cresce conforme a decisão do `07` sobre o que é medido × calculado. Cobre RF37, RF38 e RN34, e FNC005 itens 4–6. **Abre com o spike que resolve a decisão aberta do `07`** ("grandezas medidas × calculadas pelo medidor") — sem essa resposta não dá pra fechar o schema: persistir um campo derivado como se fosse medido, ou recalcular o que o próprio medidor já entrega pronto, são erros de direção opostos e igualmente caros de reverter depois que houver histórico gravado. **Depende de:** Fase 24 (a aba vive dentro de Análise) e da decisão do `07`. **Risco:** alto — mesma classe da Fase 19 (mexe em pipeline usado por todo medidor já em produção). As fases seguintes deste bloco **não** dependem desta — se a decisão demorar, o roadmap segue.
...
- **Sem replanejamento das próximas fases nesta sessão (decisão do usuário):** este fechamento cobre só a Fase 24. A Fase 25 (Telemetria ampliada) é a próxima em sequência, mas segue bloqueada pela decisão em aberto do `07` ("grandezas medidas × calculadas pelo medidor"). Como já registrado em "Duas decisões do `07` viraram bloqueios explícitos, não fases órfãs" (Replanejamento de 2026-09-06), nenhuma fase depois da 25 neste bloco depende do resultado dela — então destravar a 25 (abrindo pelo spike que a decide), ou adiantar a Fase 26 (Histórico e comparações, que depende só da Fase 23) seguem como opções em aberto para quando o detalhamento da próxima fase for retomado.
