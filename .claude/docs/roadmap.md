...
> Última atualização: 2026-08-23 · Fases 1–13.7 concluídas — épicos #94, #104, #110, #114, #128, #132, #133, #134, #148, #154, #159, #185, #187, issue #127 e PRs #250 (13.6), #254/#256 (13.7). Fase atual: **14** (conformidade P1 — governança, retenção, DSAR e consentimento), detalhe abaixo.
>
...
| **14** | **Conformidade P1** — governança (ROPA/RIPD), retenção, DSAR, consentimento e documentos legais | **Planejada — fase atual, objetivo revisado abaixo** |
...
> **Atualização de 2026-08-23 — a primeira linha encolheu.** A **ADR-0013** removeu os dois mecanismos de keep-alive: o ambiente deixou de ser demo pública (ADR-0012) e passou a hibernar por desenho, então não processa mais requisição 24/7 sem uso real. Isso reduz materialmente o achado de conformidade (registro de acesso acumulado o tempo todo) e o de desempenho A-04, e tira a data de validade da pressão sobre o limite de 0,5 GB do Neon. **Não fecha o item** — a política de retenção de `meter_readings` continua sendo escopo da Fase 14 / issue #236 — e o drift Q-29 (ADR-0011 não indexada) foi corrigido junto, com a `0011`, a `0012` e a `0013` agora presentes nos quatro índices de ADR. As demais linhas da tabela permanecem como o laudo as registrou.
...
Cobre o escopo original (retenção de `MeterReading`/`AlertTriggerEvent`/`MfaBackupCode`/`TariffFlagHistory` + política de conta inativa; export DSAR completo — consumo agregado, medidores, disparos — + PDF na UI + limite de janela do audit log; base legal por operação, aceites separados e reaceite via `consentVersion`; `RUNBOOK_INCIDENTES.md` corrigido; guarda de registros de acesso; TLS obrigatório no SMTP), **revisado com evidência concreta do laudo de 2026-08-22**: **ROPA reescrito** — hoje autocontraditório (as 7 operações afirmam "sem operador, sem transferência internacional" enquanto a tabela de operadores da mesma página lista Render/Neon nos EUA); acrescentar `email_changes`, backups e os serviços avaliados sem dado pessoal (ANEEL; UptimeRobot **não** entra mais aqui — o monitor do staging foi removido pela ADR-0013, ver item de monitor externo abaixo, que é sobre um mecanismo futuro e distinto, para a produção). **RIPD reavaliado** — risco 6.2 (credencial de medidor em claro) fechar, já corrigido; risco 6.3 (transferência internacional) reabrir com a natureza correta. **Adesão aos DPAs de autoatendimento** de Render/Neon **e** avaliação de manter ou não o Neon como staging de longo prazo, agora que a produção real está na VPS (issue #236 do épico #238 — retenção de `MeterReading` — já criada, entra aqui). Aviso de privacidade completado (cookies, prazos reais, Art. 20, Art. 14) — a autocontradição do § 5 já foi corrigida na Fase 13.6, isto é o restante.

**Acrescentado em 2026-08-23 (levantamento de pendências não tratadas):**

- **DPA do provedor de infraestrutura da VPS.** O texto acima só previa os DPAs de Render/Neon, escritos quando a produção ainda era o Caminho A. A revisão de código do PR #254 estabeleceu que o provedor que aluga a VPS **é operador** (Art. 5º, VII e X — armazenar por conta do controlador é tratamento, mesmo sem acessar conteúdo), e o `09`/`ROPA.md` já registram a lacuna. Diferente dos outros dois, este processa integralmente no Brasil: não há SCC a celebrar, só o contrato de tratamento.
- **SCC do staging, explicitamente.** Render e Neon operam nos EUA sem Cláusulas-Padrão Contratuais (Resolução CD/ANPD nº 19/2024) — lacuna reconhecida e assumida no `09`, nunca sanada. A fase decide entre celebrar, aceitar formalmente com justificativa registrada, ou eliminar o fato gerador (a mesma avaliação de "manter ou não o Neon" acima).
- **Monitor externo para a produção — decidir, não só implementar.** A ADR-0009 aceitou como custo que o Uptime Kuma **cai junto com a VM que deveria vigiar**: hoje a produção não tem como detectar a própria queda. O bloqueio nunca foi técnico, e sim a pergunta de conformidade "um ping externo em `/health` configura transferência internacional?" — que a **ADR-0011 já respondeu que não**, com raciocínio preservado de propósito quando a ADR-0013 a substituiu. Entra aqui, e não numa fase de infra, porque o que falta é a decisão jurídica, e é esta fase que reabre a análise de transferência internacional de qualquer forma. Fechar isto também fecha a metade que falta do buraco de observabilidade — a outra metade (Kuma sem canal de notificação) é configuração de minutos, fora de fase. **Ressalva técnica que a decisão jurídica sozinha não resolve:** o raciocínio da ADR-0011 foi escrito para o `/health` do Render, alcançável publicamente; na VPS, `deploy/Caddyfile` só expõe `handle /api/*` — `/health` hoje cai no fallback do SPA (confirmado no deploy da Fase 13.7, `CHANGELOG.md`). Adotar um monitor externo aqui exige primeiro expor o endpoint no `Caddyfile` (ou mirar outro caminho já público) e então revalidar se as mesmas premissas (sem autenticação, sem log de acesso, sem dado pessoal) se sustentam para o que for exposto — não é só assinar a decisão e apontar o UptimeRobot para o domínio.
