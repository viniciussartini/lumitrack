...
- **Monitor externo de disponibilidade para a produção — mantido, não depende da decisão acima.** A ADR-0009 aceitou como custo que o Uptime Kuma cai junto da VM que deveria vigiar; o bloqueio é a pergunta de conformidade "um ping externo em `/health` configura transferência internacional?" (a ADR-0011 já respondeu que não, preservado pela ADR-0013). Exige primeiro expor `/health` no `deploy/Caddyfile` (hoje só expõe `handle /api/*`, `/health` cai no fallback do SPA) e então revalidar as mesmas premissas para o que for exposto.
