...
- RF01: o sistema deve permitir que um visitante se cadastre como pessoa física (nome, e-mail, senha, CPF) ou jurídica (razão social, e-mail, senha, CNPJ), registrando consentimento LGPD versionado.
...
**Segurança**
- RNF01: rate limit global de 1000 requisições / 15 min por IP; rate limit estrito de 10 requisições / 15 min por IP em `/login`, `/login/mfa`, `/forgot-password` e `/reset-password`.
- RNF02: senhas com bcrypt (12 rounds); CPF, CNPJ, endereço e segredo MFA cifrados em repouso com AES-256-GCM (chaves segregadas por finalidade); refresh token e token de sessão armazenados como hash SHA-256, nunca em texto claro.
...
- RNF04: HSTS com 1 ano + subdomínios; CSP deny-all (API JSON pura); `CORS_ORIGIN` não pode ser `*` em produção (guardado por schema).
...
**Qualidade e entrega**
- RNF07: o pipeline de CI (`.github/workflows/ci.yml`) deve bloquear o merge em caso de falha de lint, build, testes ou `npm audit --audit-level=high`, para backend e frontend, além da suíte E2E Playwright.
