...
- RN39 `[implementado]`: a comparação ACR × ACL recalcula o mesmo consumo real medido, mês a mês, nos dois cenários (RN38 para o ACL; catálogo + bandeira para o ACR) — nunca estimativa. O **PLD é contexto informativo** do submercado do contrato no período, exibido junto do resultado, **não entra na fórmula de custo** de nenhum dos dois cenários. Validado em `consumption.service.ts` (`compareAclToAcr`).
