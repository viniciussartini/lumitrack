...
- [ ] **Contrato de tratamento (DPA)** com cada operador. **Estado atual (ADR-0010 + ADR-0012, Fase 13.7): dois ambientes, um só com operador.** A **produção** (VPS Hostinger, São Paulo) não tem nenhum operador — zero DPA a assinar. O **staging/validação** (Render + Neon, EUA) continua com dois operadores sem DPA. **Lacuna reconhecida, não resolvida**: é aceita porque esse ambiente é de validação (cadastro fechado, apenas contas sintéticas) — diferente do que a ADR-0010 previa originalmente, a lacuna não "some quando a hospedagem migrar" (a migração já aconteceu, para a produção; o staging continua de propósito). Inventário vivo na tabela de operadores de `.claude/docs/ROPA.md`. Nenhum provedor SMTP contratado em nenhum dos dois ambientes.
...
- [ ] **Estado atual (ADR-0010 + ADR-0012, Fase 13.7): aplica-se só ao staging, de forma limitada e sem SCC — a produção não tem transferência internacional.** A **produção** (VPS Hostinger, São Paulo, `lumitrack.app.br`) roda inteira no Brasil, sem operador — sem transferência, por inexistência do fato gerador, restaurando a conclusão da ADR-0008. O **staging/validação** (Render + Neon, EUA) continua com exposição: o único dado pessoal que sai do Brasil ali são os **registros de acesso de visitantes** (IP, data/hora, rota) — as contas da aplicação são sintéticas e o cadastro está fechado nos dois ambientes, então não há dado de titular real no produto. **Não há SCC celebrada**: lacuna reconhecida e assumida como risco do ambiente de validação, não sanada. **Gate:** abrir cadastro para usuários reais em qualquer um dos dois ambientes reabre esta análise para aquele ambiente especificamente.
