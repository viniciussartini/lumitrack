...
- [ ] **Contrato de tratamento (DPA)** com cada operador. **Estado atual (ADR-0010 + ADR-0012, Fase 13.7): dois ambientes, três operadores, nenhum DPA assinado.** A **produção** (VPS, São Paulo) tem **um** operador — o provedor de infraestrutura que aluga a máquina: armazenar dado por conta do controlador é tratamento (Art. 5º, X) e quem o faz é operador (Art. 5º, VII), independentemente de não acessar o conteúdo da aplicação. A diferença que importa em relação ao staging não é "ter ou não ter operador", é que **esse processa integralmente no Brasil** — sem transferência internacional, sem SCC a celebrar. O **staging/validação** (Render + Neon, EUA) tem dois operadores, sem DPA e com transferência internacional descoberta. **Lacunas reconhecidas, não resolvidas**: aceitas porque nenhum dos dois ambientes tem titular real (cadastro fechado, contas sintéticas) — e, diferente do que a ADR-0010 previa, a do staging não "some quando a hospedagem migrar" (a migração aconteceu; o staging continua de propósito). Inventário vivo na tabela de operadores de `.claude/docs/ROPA.md`. Nenhum provedor SMTP contratado em nenhum dos dois ambientes.
