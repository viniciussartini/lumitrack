...
> **Lacuna de observabilidade — metade fechada:** a produção é vigiada por um monitor que roda **dentro da própria máquina** que vigia — se a VPS cair, o Kuma cai junto e ninguém é avisado (custo aceito explicitamente na `ADR-0009`). A **ADR-0015** fecha a detecção externa: `/health` exposto via Caddy + monitor externo (ex.: UptimeRobot), reaproveitando o raciocínio de conformidade da `ADR-0011`. Falta só a ação do usuário fora do repositório (criar o monitor e aplicar o `Caddyfile` em produção); o canal de notificação do Kuma continua pendente, à parte.
