...
- **Observabilidade:** pino + pino-http (logs estruturados). No **Caminho B** (self-hosted), monitor de uptime **Uptime Kuma auto-hospedado** (`ADR-0009`, container do `docker-compose.yml`, painel só acessível via túnel SSH) monitorando `/health`. No **Caminho A** (demo no Render) o Kuma não é usado — vale o health check nativo da plataforma (`healthCheckPath: /health` no `render.yaml`) mais o **UptimeRobot** (`ADR-0011`, monitor externo gratuito) fazendo o keep-alive contra a hibernação por inatividade do free tier. Sem Sentry/APM nem analytics de produto.
...
| `0011` | Observabilidade | Keep-alive da demo via UptimeRobot (monitor externo) |
