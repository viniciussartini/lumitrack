...
- **Infra/deploy:** VM única na **Oracle Cloud Always Free, região São Paulo** — backend, frontend estático, PostgreSQL e o simulador IoT co-locados, sem operador estrangeiro (**ADR-0008**, decisão tomada como conformidade: elimina a transferência internacional em vez de contratar SCC). Nenhum artefato de deploy existe no repositório ainda (sem Dockerfile, sem unidade `systemd`, sem config de reverse proxy; `backend/package.json` tem `prisma migrate dev`, mas não o `migrate deploy` de produção) — a ADR define a topologia e os gates de go-live, e a **Fase 13.5 do roadmap** é quem produz os artefatos e fecha os gates #6 (backup testado) e #7 (rotação de chaves).
