...
- **Cross-cutting:** tudo em `backend/src/shared/` (detalhado abaixo) — auth/authz em `middlewares`, erro no error handler central, log em `logger` (pino), config por ambiente com fail-fast no boot, auditoria em `audit`. Sem i18n e sem feature flags além de `REGISTRATION_ENABLED`.
