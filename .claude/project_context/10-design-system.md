...
>
> **Escala tipográfica e de espaçamento resolvida (Fase 18, issue #336, 2026-08-30).** O `@theme` mapeia cor, fonte, raio e sombra desde a Fase 1; o espaçamento já era coberto pelo multiplicador `--spacing: var(--space-1)` (mesma fase — estende o ritmo de 0,85× a toda a escala numérica do Tailwind, `p-1`..`p-96`), mas não estava documentado aqui, o que por si só já era uma causa de valor arbitrário por desconhecimento. A lacuna real era a **tipografia**: sem uma escala nomeada, 145 das ~372 ocorrências de valor arbitrário (medição de 2026-08-30) eram `text-[Npx]`. Resolvida com uma escala aditiva de 26 tokens (`--text-10` a `--text-44`, incluindo os passos de meio pixel que o bundle usa sistematicamente — 12.5, 13.5, 14.5...), levantada da união de `font-size` de todos os `.dc.html` do bundle vigente, em `frontend/src/styles/industry.css`/`index.css`. Coexiste com a escala nomeada padrão do Tailwind (`text-xs`/`sm`/`lg`...), que o projeto já usa em 141 pontos sem correspondente no protótipo — não substituída. Além disso: `--color-status-live` (o verde do indicador "ao vivo", cor mais repetida do app, 9 pontos) e os 7 tokens de bandeira tarifária sobre fundo escuro (`--color-flag-*`) também criados. **Aplicação nos 44 arquivos com valor arbitrário e nos 17 com hex hardcoded é a issue seguinte** (épico #334) — os tokens acima existem, mas ainda não foram consumidos em massa. Tokens pré-Industry residuais: 13 arquivos (issue #342).
