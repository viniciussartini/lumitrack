...
>
> **Escala tipográfica e de espaçamento (Fase 18, épico #334, issues #336-#340 e #353, 2026-08-30).** O `@theme` mapeia cor, fonte, raio e sombra desde a Fase 1. Tipografia: escala aditiva de 26 tokens (`--text-10` a `--text-44`, incluindo os passos de meio pixel que o bundle usa sistematicamente — 12.5, 13.5, 14.5...), levantada da união de `font-size` de todos os `.dc.html` do bundle vigente. Coexiste com a escala nomeada padrão do Tailwind (`text-xs`/`sm`/`lg`...), que o projeto já usa em 141 pontos sem correspondente no protótipo — não substituída. Espaçamento tem **duas camadas**, achado só percebido ao investigar a fundo: o multiplicador `--spacing: var(--space-1)` (Fase 1, 3.4px×N) cobre só os componentes de base do Industry (`.btn`/`.card`/`.dialog`) — o layout de página no bundle usa medida livre, sem grade (conferido nos 8 `.dc.html`: `gap:12px`, `padding:18px`... nenhum múltiplo de 3.4px, e isso não é dívida, é como o design é). Resolvido com uma segunda escala aditiva de 8 tokens de alta frequência (`--spacing-9px/15px/22px/26px/30px/46px/1200px`, os únicos com 8+ ocorrências reais no código) — o resto (~31 valores, 1-4 usos cada) fica arbitrário por escolha, sem repetição que justifique nome. **Nomes com sufixo `px` obrigatório:** um nome puramente numérico (`--spacing-9`) colide com o namespace que o multiplicador `--spacing` já resolve dinamicamente para qualquer número (`w-9` já existia via `calc(var(--spacing) * 9)`) — achado corrigido depois de passar despercebido por uma rodada de revisão de código, que encolheu 11 usos pré-existentes não tocados pela mudança. Também criados: `--color-status-live` (verde do indicador "ao vivo") e 7 tokens de bandeira tarifária sobre fundo escuro (`--color-flag-*`), já aplicados em `tariff-flag.types.ts`. **Estado final da aplicação:** 372→60 valores arbitrários (84% de redução), 44→34 arquivos, hex hardcoded 66→50 (17→~13 arquivos). O resto (espaçamento sem token) é exceção catalogada no lint anti-regressão (item 7 da Fase 18, implementado em `frontend/eslint.config.js`).
>
> **Blueprint vs. cantos manuais (Fase 18, épico #335, issue #345, 2026-08-30).** Critério único: `.blueprint` (borda 1px + 4 marcas de registro `+` nos cantos) veste todo elemento que é, ele mesmo, um cartão/painel autônomo na composição da página — mesma regra do bundle ("Frame cards, figures and primary buttons as blueprint objects", `design-system/industry-readme.md`). Dois carve-outs, categorias diferentes da regra, não exceções a ela: **(1) conteúdo aninhado dentro de um `.blueprint` que já existe** não ganha moldura própria — herda o enquadramento do pai; achado real por trás da decisão foi justamente isso: `ConsumptionChart`/`ConsumptionTable` têm borda arredondada própria mas só renderizam dentro de `ConsumptionSection`/`ConsumptionHistorySection`, já em `.blueprint` — borda dupla, não um segundo cartão. **(2) estados vazios/de espera** (borda tracejada, skeleton) são convenção própria do projeto, não vêm do bundle, e sinalizam "sem conteúdo" — marca de registro nesse contexto comunicaria o oposto (`PlaceholderPage.tsx`, as faixas "sem dados"/"em breve" de `ConsumptionChart.tsx`/`ReportsPage.tsx`: ficam como estão). Levantamento (`rounded-lg`/`rounded-xl` como sinal — a Industry é "square-cornered", nunca cantos arredondados em cartão): 4 arquivos divergentes, migrados — `ReportsPage.tsx` (painel de filtro, era `rounded-lg border bg-surface`; único caso achado que É um cartão autônomo e não usava o tratamento, virou `<Blueprint>`); `ConsumptionChart.tsx`/`ConsumptionTable.tsx` (carve-out 1, borda removida, não substituída); `PlaceholderPage.tsx` (carve-out 2, sem mudança).
>
> Tokens pré-Industry residuais: 13 arquivos, migrados — 12 na issue #342 e o 13º (`UserMenu.tsx`, coordenado com a remoção do ramo morto) na issue #344 (Fase 18, épico #335, 2026-08-30).
