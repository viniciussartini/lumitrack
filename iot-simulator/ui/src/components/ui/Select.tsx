    ...
    ({ label, error, helperText, className, id, children, ...rest }, ref) => {
        ...
        return (
            <div className="flex flex-col gap-1.5">
                ...
                {hasError ? (
                    <span id={`${selectId}-error`} role="alert" className="text-xs text-red-600 dark:text-red-400">
                        {error}
                    </span>
                ) : helperText ? (
                    <span id={`${selectId}-helper`} className="text-xs text-slate-500 dark:text-slate-400">
                        {helperText}
                    </span>
                ) : null}
            </div>
