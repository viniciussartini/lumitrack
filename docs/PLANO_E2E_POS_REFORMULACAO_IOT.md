...
- `reports.spec.ts` ✅ **Concluído (17/07/2026)** — `/relatorios`: cascata `reports-property-select` → `reports-area-select` → `reports-device-select` e o `targetType` resultante (DEVICE > AREA > PROPERTY) na query de `/api/consumption`.

  **Nota de implementação:** 3 testes, 6/6 verdes nos dois browsers de primeira, 54/54 na suíte completa. Como `ConsumptionSection` é remontada a cada troca de alvo (`key={targetType-targetId}`), a precedência é provada inspecionando a query real de `/api/consumption` (não só o valor dos selects) — selecionando os três níveis simultaneamente e confirmando que `DEVICE` vence mesmo com área/propriedade ainda selecionadas. Sem `useDistributors` nesta página (só nome das entidades nos selects). `meters/by-target` mockado como "medidor presente" universalmente — inverso do padrão das sub-issues #4/#7, necessário aqui pra `/api/consumption` disparar de verdade. Detalhes no log.
